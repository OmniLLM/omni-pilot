# floating-panel-accessibility Specification

## Purpose

Define how the floating content-script panel — the in-page assistant that OmniPilot injects into an open Shadow DOM on any site — is exposed to assistive technology.

The floating panel and the side panel render the same conversation, so they must be announced the same way. This capability holds the floating panel to the contract the side panel already meets: a single status live region whose role is stable and whose urgency is carried by `aria-live`, an explicit busy state while a response is streaming, and decorative brand glyphs kept out of the accessibility tree so that every conversation turn in the `role="log"` transcript is announced as its speaker and content, nothing more.

Scope is presentation only. Nothing here governs streaming transport, provider routing, session persistence, or A2A delegation, all of which remain frozen functional logic under hard constraint 6.
