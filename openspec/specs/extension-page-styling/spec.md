...
Adopting the CSS framework SHALL NOT alter any JavaScript control flow, event handling, storage access, or message passing, and SHALL NOT change the build pipeline that produces `dist/*.js`.
...
- **WHEN** the migrated extension page markup is inspected
- **THEN** every element `id` referenced by `getElementById` in the corresponding `.mjs` entry file is unchanged

#### Scenario: JavaScript bundling is unchanged

- **WHEN** `build.mjs` is inspected
- **THEN** its JavaScript concatenation and `export`-stripping logic is unmodified, and no bundler wraps the output in an IIFE
